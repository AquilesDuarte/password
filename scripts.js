const passInputed = document.querySelector(".input-group input");
const toggleIcon = document.querySelector(".input-group .toggle");
const ripple = document.querySelector(".input-group .ripple");
const percentBar = document.querySelector(".strenght-label");
const passLabel = document.querySelector(".strength-label");

passInput.addEventListener("input", handPassInput);
toggleIcon.addEventListener("click", togglePassInput);

function handPassInput(e) {
    if(passInputed.value.length = 0){
        passLabel.innerHTML = "Strenght"; addClass();
